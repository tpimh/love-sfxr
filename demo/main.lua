...
function createActionButtons()
    ...
    local locate = lf.Create("textinput", fr)
        :SetPos(5, 270)
        :SetWidth(390)
        :SetText(love.filesystem.getSaveDirectory() .. "/sounds/")
        :SetEditable(false)
    ...
    local sb = lf.Create("button")
        :SetText("Save Lua")
        :SetWidth(67)
    sb.OnClick = saveHandler("sfxr", function(f) sound:save(f, true) end)
    f:AddItem(sb)

    local lb = lf.Create("button")
        :SetText("Load Lua")
        :SetWidth(67)
    lb.OnClick = loadHandler("sfxr", function(f) sound:load(f) end)
    f:AddItem(lb)
